    ...
    <div className="glass-card overflow-hidden bg-white border-slate-200 shadow-sm">
      ...
      <div className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {stats.map(([category, data]) => (
          <div key={category} className="group p-4 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col items-center text-center hover:border-amber-500/20 transition-all duration-300 shadow-sm hover:shadow-lg">
            <div className="p-3 bg-white rounded-xl text-amber-600 mb-3 group-hover:scale-110 transition-transform duration-300 shadow-sm">
              {getIcon(category)}
            </div>
